...
- Statement coverage is 99.7%, and the one uncovered statement is
  ...
  `FuzzAppendUnescapeAgainstRef` 30 s, no new failures).
...
Table currency, as of 2026-08-22: the committed tables are **current through
the 2026-08-17 amd64 review** — `a1000a4` regenerated them at 16:44Z with
`escClean` 5, the `hex4` table and the probe hint in. (Two earlier versions of
this paragraph each declared a staleness that a regeneration had already
fixed; check the `generated` stamp in the file against `git log -- bench/`
rather than trusting this line.) They are **stale for the 2026-08-22 seeded
value scan**, which is worth a deliberate `bench.yml` dispatch — its arm64
table doubles as the first cross-arch indication for that pass. The CI arm64 numbers
...
gives a first indication either way.
...
parse per field). The long-value scan rows still stand.
...
- **The key scan seeds the value scan** (2026-08-22 — the 2026-08-17 parked
  ...
  4. A narrow single-word key loop covers the record's last 8..15 bytes,
     ...
     none on the hot one).
  Quoted values pay one extra compare (the seeded quote test jumps to
  `quotedVal`, which re-tests `data[i]`); separator runs and bare keys pay
  nothing new. Fuzz seeds pin the view-boundary shapes: stops at base+15 and
...
- **2026-08-17 pass (Neoverse N2, arm64) — measured and rejected.** Every item
  ...
  - *Lookahead-seeded value scan* — load `data[i+8:i+16]` in the key loop as
    ...
    Still unmeasured on the N2, where IPC 4.41 leaves less width to absorb
    the extra parallel work.
