...
- `logfmt_swar_test.go` — `FuzzIterateAgainstRef`: differential fuzz of the
  ...
  lower of two stops wins" — `hasQuoteOrEsc` gets that second part with a `\`
  immediately before a `"`, the shape its scan walks through constantly) and
  `Test_Unit_IsSpace`. The 2026-08-14 seeds reach the escape-dense scan and both
  of its mode transitions; one of them is the input that caught the hand-back
  leaving `i` at `n+1`, where the outer `data[i:]` panics rather than merely
  failing to match. That same input is also checked in under
  `testdata/fuzz/FuzzIterateAgainstRef/` — Go's own regression-corpus mechanism,
  written there by the fuzzer when it crashed and run automatically by plain
  `go test` from then on. Leave it committed. `isSpace` needs its own test
...
- `getmany_fuzz_test.go` — `FuzzGetManyAgainstRef`: differential fuzz of
  ...
  **Run after any change to the lookup state machine.** Caveat worth knowing: it
  ...
  `AppendUnescape` cancels on both sides. That hole is covered since 2026-08-14
  by `unescape_fuzz_test.go` below, not by this file — keep both.
- `unescape_fuzz_test.go` — `FuzzAppendUnescapeAgainstRef`: differential fuzz of
  `AppendUnescape` against a byte-at-a-time reference, added 2026-08-14 to close
  the gap the note above describes. Until then the decode half of the API had
  **no independent oracle at all** — `FuzzGetManyAgainstRef` checks `AppendValue`
  by calling `AppendUnescape` on the other side, so a bug inside `AppendUnescape`
  cancelled out and was invisible, which is the worst place for one, since a
  search bug there returns plausible bytes rather than failing. It also checks
  that the result APPENDS cleanly onto a non-empty `dst` (an offset bug can look
  right when `dst` is empty) and that `NeedsUnescape` never says "no decode
  needed" about a value the decode would change. Like `iterateRef` and `isSpace`,
  it shares one thing with the code under test — `decodeUnicodeEscape`, hence the
  surrogate and malformed-`\u` rules — which is exactly why
  `Test_Unit_Unescape_Unicode` tests that piece directly. Validated by running it
  against the UNCHANGED `AppendUnescape` first (23.7 M execs) before anything was
  optimized, so it is known to be an oracle rather than a copy of a new bug.
- `*_test.go` — unit tests, benchmarks, and a regex-vs-logfmt comparison.
  ...
  decode split. `Test_Unit_Quoted_EscapeDense_Scan` pins the two-mode quoted
  scan — both modes, both transitions between them, and the two malformed
  shapes only the dense mode can reach — sizing its inputs off `denseGap` so it
  follows the constant instead of a stale copy of it.
  `Benchmark_IterateEscaped` sweeps escape density at fixed
  length, and `Benchmark_UnescapeEscaped` (2026-08-14) sweeps the same
  densities through `AppendUnescape`, over bytes from the same generator so the
  parse and decode halves of one shape are comparable.
...
Table currency, as of 2026-08-14: the committed tables are **current through
`e048e5d`** (the −13.8% pass — `dcd5687` is its direct child, stamped 95 s
later) and **stale for `3a55c5e`, `c04fbed` and everything after**, the
two-mode-scan pass included. Note `bench.yml` is `workflow_dispatch` only, so
nothing refreshes them automatically; run `make bench-md` deliberately. Don't
hand-edit them with laptop figures — and note that regenerating
`pkg_results_amd64.md`/`results_amd64.md` on the 2026-08-14 box would not just
refresh the numbers but re-stamp them with a different CPU (Intel Core Ultra 9
185H, where the committed ones are AMD), so do it deliberately or not at all.

2026-08-14 pass — the two-mode quoted scan. n=6 interleaved pinned rounds per
series on an Intel Core Ultra 9 185H (P-core 6), A/A control clean at +0.36%
geomean with every row `~`, and the bench module's three comparison libraries
`~` throughout as an embedded control. Baseline (HEAD, this box, for the ratios
only): `Iterate` 253.6 ns, `GetMany` 45.4, `Unescape` 18.5, `DecodeKeyval`
468 µs, `LevelTS` 39.7, `IterateEscaped` 23.1 / 82.8 / 209 / 743 / 2823 ns
across esc=0/8/32/128/500.

| Benchmark | Δ |
|---|---:|
| `IterateEscaped/esc=128` | **−33.0%** |
| `IterateEscaped/esc=500` | **−32.8%** |
| `IterateEscaped/esc=32` | +1.5% |
| `IterateEscaped/esc=8` | +2.5% |
| everything else in both suites | `~` |

The two positive rows are the whole price of the mode test — one compare per
escaped quote, on values whose escapes are too far apart to benefit — and they
were weighed against the alternatives in Rejected below rather than accepted
blind. Nothing on the realistic suite moved in either direction.

One caveat on the series *after* that one, which is worth knowing before
trusting any of this session's later numbers: the desktop came back to life
partway through (Chrome, VLC, an editor), and the tell was immediate and
exactly the one this file predicts — the bench module's comparison libraries,
which no change here can touch, started reporting +8% and +7%. Every number
quoted above comes from a series whose embedded control was clean; the ones
whose control was not are named as such where they appear.
...
| Quoted value scan, **per escaped quote**, IndexByte mode | ~5.6 ns each (see below) |
| Quoted value scan, **per escaped quote**, dense mode | ~3.8 ns each (2026-08-14) |
...
SWAR steps — **that description is now only true of the sparse path**; see
below. Embedded JSON in a `msg=` field is the worst realistic shape: every JSON
quote becomes `\"`, i.e. an escape roughly every two bytes.
`Benchmark_IterateEscaped` pins that axis; it exists because `sample_big.txt`
has 2 escaped quotes in 1.4 KB (~4%) and so makes the quoted scan look like pure
`IndexByte` throughput.

**The adaptive fix that used to be parked here has landed** (2026-08-14; see
"The quoted-value scan has two modes" above). At high density the scan no longer
restarts `IndexByte` per escape but reads a word at a time inline. Measured on
this session's box — an Intel Core Ultra 9 185H, **not** the Ryzen the rest of
this file's absolutes come from, so compare only within this paragraph — the
1 KB sweep sits at 23.1 ns with no escapes, and 500 escapes went 2823 →
**1922 ns**, i.e. the density penalty fell from ~122× to ~83×; 128 escapes went
743 → **502 ns**. What is left is the per-escape *decision*, not the scan: even
inline, the loop still stops once per escape.

The `AppendUnescape` half of the same shape is pinned too, by the new
`Benchmark_UnescapeEscaped` — and it turns out to be **the slower half**: 23.7 ns
with no escapes and 3257 ns at 500 of them (~137×), against the parser's 1907 ns
for the same value. It still restarts `bytes.IndexByte` at every escape exactly
as the parser used to, so at high density decoding a value now costs more than
parsing it did. The same two-mode fix was built and measured there and is
**parked, not landed** — it wins −33%/−30% on the dense cases but costs 5% on
`GetMany`, which never executes a byte of it; see Rejected below for the numbers
and for the two shapes it was tried in.
...
## The `iterate` / `Iterate` split — MEASURED 2026-08-14: leave it alone

The question this section used to ask ("what does the adapter hop cost?") now
has an answer: **about 1% geomean for the whole split, and none of it is the
adapter.** Do not spend the breaking API change on it.

Method, in case it needs redoing: a tree was built from HEAD with the split
mechanically undone and nothing else touched — `iterate`'s callback loses its
third argument, the `quoted` local disappears, and `Iterate`/`All` pass their
callbacks straight through with no adapter closure. That tree is a measurement
artefact and must never be landed: with the flag gone `GetQuoted` reports false
unconditionally and `AppendValue` silently stops unescaping, which is the bug
the split exists to fix. Its unit tests are expected to fail. (The script that
generates it is trivial enough to rewrite; both fuzz files have to be dropped,
from *both* trees, since they reference the three-argument callback — and the
same test files must be present on both sides or the test binaries differ in
layout and the comparison measures that instead.)

n=6 interleaved pinned rounds, root suite, the no-split tree as B:

| Benchmark | Δ for removing the split | uses the adapter? |
|---|---:|---|
| `IterateOur` (29 fields/call) | `~` | **yes** |
| `DecodeKeyval_Custom` (~60 k fields/call) | `~` | **yes** |
| `GetMany_TimestampLevel` | `~` | no |
| `Unescape`, `esc=32/128/500` | `~` | — |
| `IterateEscaped/esc=0` | −3.05% | yes |
| `IterateEscaped/esc=8` | −1.96% | yes |
| `LevelTS_LogFmt` | −4.86% | no |
| geomean | −1.12% | |

The reading is in the third column. If the adapter's extra indirect call per
field cost anything, the two benchmarks that call it tens of thousands of times
would show it first, and both are flat. What moves instead are a lookup that
never touches the adapter at all and two single-field lines — which points at
carrying the flag, at code layout, or at both, not at the hop. `Iterate` is
therefore fine as a thin adapter, and the three-argument `Iterate` that was
floated as the "fix" would be a breaking change bought with nothing.

Deriving `quoted` at the callback site as `data[vStart-1] == '"'` instead of
carrying a live flag is still available and still unmeasured; it is worth ~1% at
the absolute most, and it is the only one of the two ideas that costs nothing at
the API. Do not duplicate the parser to chase this; that has been tried and
rejected (see below).

Two things remain true and were re-confirmed here, deterministically from
`-gcflags=-S`/`-m`: both SWAR scan loops are unchanged by the split, and the
adapter closure does not escape, so the zero-allocation contract holds — pinned
by `Test_Unit_HotPath_Allocs`.

### Original note (2026-08-08), kept for the reasoning
...
At the time this landed the cost of the adapter hop could not be measured: an
unrelated ffmpeg job held ~10 cores for the whole session (load average 15–50),
and a four-way interleaved sweep with an embedded A/A control put the control
itself at −7.8% / +3.75% — far above any effect worth finding, so by this file's
own control rule none of those numbers counted. Six days later a quiet box
answered it; see the table above. Worth keeping from the original note, since it
was established deterministically rather than by stopwatch: **both SWAR scan
loops are byte-identical to the pre-split code** (same two `TZCNT` sites, same
loop bodies), so only the per-*field* term could ever have moved — which is
exactly what the measurement then found had not moved either.

`vStart` is only ever set just past a `"` or a `=`, so `data[vStart-1]` is
already hot if anyone does want to try the derive-at-the-callsite variant; a
working version was built and passed the suite back then.
...
- **The quoted-value scan has two modes** (2026-08-14). `bytes.IndexByte` opens
  every quoted value and is unbeatable on a clean one — one SIMD call, one scan,
  done, ~44 GB/s on this box. Its cost model only breaks on an escape-DENSE
  value, where every `\"` restarts the non-inlinable call over a span a byte or
  two long, so the cost is O(escapes) calls rather than O(bytes/8) inline steps.
  Embedded JSON in a `msg=` field is exactly that shape. So the scan switches to
  an inline SWAR scan for `"` and `\` (`hasQuoteOrEsc`) once it has seen an
  escaped quote **and** the gap before it was `<= denseGap`, and switches back as
  soon as the gaps grow. In dense mode a `\` simply consumes the byte after it —
  stepping over the pair *is* the run-parity rule — so the backslash-run walk is
  not needed there at all. Measured (n=6 interleaved pinned rounds, A/A control
  clean, root + bench suites): `IterateEscaped/esc=128` **−33.0%**, `esc=500`
  **−32.8%**; `Iterate`, `GetMany`, `DecodeKeyval`, `LevelTS`, `Unescape`,
  `ParseAll_Big`, `ParseAll_Typical`, `ParseEscaped`, `Extract` and all three
  comparison libraries `~`. The cost is `esc=8` +2.5% and `esc=32` +1.5%: one
  compare per escaped quote, on the values whose escapes are too far apart to
  benefit. Both the threshold and the decision to put the dense scan in its own
  function are measured — see Rejected below for what each alternative cost.
...
- **2026-08-14, the three losing shapes of the two-mode quoted scan.** All were
  correct and fuzz-clean; each lost on the stopwatch, and together they are why
  the landed version looks the way it does:
  - *One always-dense scan* — on seeing any escaped quote, switch to SWAR for
    the rest of that value, no threshold. −32% at 8-byte gaps and −32% at
    2-byte ones, but **+48% at 128-byte gaps**, +6.4% on `GetMany` and +4.6% on
    `LevelTS`. SWAR runs ~14 GB/s here where `bytes.IndexByte` runs ~44: below
    roughly a word of gap the call overhead dominates and SWAR wins, above it
    the SIMD throughput does. A single fixed strategy cannot have both.
  - *`denseGap = 32`* instead of 8. Catches `IterateEscaped/esc=32` for a
    further −9.4%, and looks strictly better on the synthetic sweep — but it
    sends the bench module's realistic escaped line (four escapes, gaps of
    18/2/2/9) into dense mode and costs it **+7.3%**, with `Extract` +3.3%.
    Four escapes are too few to amortise entering the mode. Measured both ways,
    twice; the realistic suite decided it, exactly as it did for DecodeKeyval.
  - *The dense scan inlined into `iterate`* rather than living in
    `scanEscapeDense`. Costs **`GetMany` 5.1% and `Extract` 3.1%** — benchmarks
    that never execute a byte of it — purely by growing `iterate`. This is the
    same code-growth effect that killed the split-`fn`-call-sites experiment
    above, and it is worth remembering how far it reaches: the regression shows
    up in lookups that stop after four fields. The call the separate function
    costs is paid once per dense value, never once per escape.
- **2026-08-14, the same two-mode fix inside `AppendUnescape` — built,
  correct, fuzz-clean, and PARKED.** Worth reading before rebuilding it, because
  the reason it is parked is not the reason you would expect. The decode path
  restarts `bytes.IndexByte` at every escape exactly as the parser's scan used
  to, and giving it the identical treatment (a `hasEsc` word mask, entered off
  the same `denseGap`) wins **−33% at 8-byte gaps and −30% at 2-byte ones**,
  reproduced across four series. Two shapes were tried:
  - *A `dense` flag tested at the top of the loop*: **+8.4% on escape-free
    input, +2.7% on sparse, +1.8% on `ParseEscaped_Mine`**. The branch, not the
    scan, was the whole cost — this is a function whose common case is one
    `IndexByte`, one `append`, return, so a per-iteration test is enormous
    relative to it.
  - *Falling through into the SWAR search only after decoding an escape whose
    gap was short, re-entering the shared decode with `goto decode`*: fixes all
    three of those (escape-free and sparse both back to `~`, `ParseEscaped_Mine`
    `~`), keeps the full dense win, and needs no duplicated decode logic. This
    is the right shape if it is ever revisited.
  What killed it anyway: **`GetMany` +5.15% (p=0.000, n=8), and `GetMany` never
  calls `AppendUnescape` at all.** Positive in all four series that included the
  change and `~` in the parser-only series, so it is the change — reaching
  `GetMany` purely as binary layout. Same class of effect as the inlined dense
  scan above, but from a function on a completely different call path, which
  makes it far more annoying: there is no smaller-function fix, because nothing
  about `iterate` grew. Landing it would mean paying 5% on the flagship lookup
  to speed up dense decoding, and this file's standard says no. The patch is
  reproducible from this description in about twenty minutes; the missing piece
  is a way to add the code without moving `iterate`.
...
- **The A/B harness itself needs three things, learned the hard way (2026-07-26)
  ...
  2. **Pin to a core** (`taskset -c 6`). This cut run-to-run variance from
     ...
     **Check which core you are pinning to on a hybrid CPU.** The box used on
     2026-08-14 is an Intel Core Ultra 9 185H, where `lscpu -e=CPU,CORE,MAXMHZ`
     reports CPUs 0–11 as P-cores (4.8–5.1 GHz), 12–19 as E-cores (3.8) and
     20–21 as low-power E-cores (2.5). `taskset -c 6` lands on a P-core there
     and gave a clean A/A control at ±1–3%, but the same command on a different
     topology can quietly park the whole series on an E-core and make every
     absolute in it meaningless. Also **check nothing else is pinned to that
     core**: two series on CPU 6 at once halve each other, which reads as a
     flat 2× regression across every row and is easy to mistake for a real one.
...
go test -run='^$' -fuzz=FuzzGetManyAgainstRef -fuzztime=20s # lookup fuzz
go test -run='^$' -fuzz=FuzzAppendUnescapeAgainstRef -fuzztime=20s # decode fuzz
